...
describe('SalesService (Unitarias)', () => {
    ...
    beforeEach(() => {
        ...
        service = new sales_service_1.SalesService(db, emailService);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('RF-003.1 - Catálogo de productos', () => {
        ...
        it('CP-075 - debe aplicar filtros por nombre, categoría y rango de precios', async () => {
            ...
            expect(db.query).toHaveBeenCalledWith(expect.stringContaining('LOWER(c.nombre) = LOWER(?)'), expect.arrayContaining(['Panadería']));
        });
    });
    ...
    describe('RF-003.6 - Confirmación de venta', () => {
        ...
        it('CP-097 - debe rechazar un método de pago no soportado', async () => {
            ...
            db.getConnection.mockResolvedValue(connection);
            await expect(service.createOrder({ items: [{ id: '1', cantidad: 1 }], total: 3500, id_metodo: 'M999' }, 5)).rejects.toThrow(common_1.BadRequestException);
        });
        it('CP-100 - debe rechazar la compra cuando el stock es insuficiente y hacer rollback', async () => {
            ...
            db.getConnection.mockResolvedValue(connection);
            await expect(service.createOrder({ items: [{ id: '1', cantidad: 2 }], total: 7000, id_metodo: 'M1' }, 5)).rejects.toThrow(common_1.ConflictException);
        ...
        it('CP-101 - debe manejar errores de base de datos con rollback', async () => {
            ...
            db.getConnection.mockResolvedValue(connection);
            await expect(service.createOrder({ items: [{ id: '1', cantidad: 1 }], total: 3500, id_metodo: 'M1' }, 5)).rejects.toThrow(common_1.InternalServerErrorException);
            expect(connection.rollback).toHaveBeenCalled();
        });
    });
});
//# sourceMappingURL=sales.service.test.js.map